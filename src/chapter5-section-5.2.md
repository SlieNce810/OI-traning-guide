...
> **学习目标**：掌握 DFS 树的边分类、Tarjan 算法求强连通分量、割点/桥的判定以及 2-SAT 的蕴含图建模——让 DFS 从"遍历工具"升级为"图结构分析核心引擎"。

## 理论基础

### 为什么需要学这个？

你可能已经用 DFS 写过无数遍图的连通性判断了——不就是标记 visited 然后递归吗？可到了强连通分量（SCC）的问题面前，光有 visited 根本不够用：你怎么知道一个点的所有后代中有没有能回到祖先的边？你怎么判断删掉哪个点会让整张图裂开？这些问题用简单的 visited 标记完全没法回答。

DFS 真正的威力在于它对图的**深度优先搜索树**所附加的**时间戳信息**。一旦你在 DFS 过程中给每个点打下 `dfn`（发现时间）和 `low`（能回溯到的最早祖先）这两个标记，整张图的结构信息就被编码到了数值中。SCC 的缩点、割点和桥的判定、2-SAT 的可满足性——这些看似无关的问题，内核用的全是同一套 "dfn + low" 机制。学透了这一节，你就拥有了一把图结构分析的"X光机"。

### 核心概念

#### 1. DFS 树的四种边——一次遍历全部看清

在 DFS 生成的有向图搜索树中，每条边恰好属于以下四类之一：

| 边类型 | 判定条件 | 含义 |
|--------|----------|------|
| **树边** | `v` 未被访问 | DFS 的前进方向 |
| **回边** | `v` 是 `u` 在 DFS 树上的祖先 | 形成环的关键 |
| **前向边** | `v` 是 `u` 的后代，但非直接儿子 | 非树边的"捷径" |
| **横叉边** | `v` 已访问完毕且与 `u` 无祖孙关系 | 跨子树的连接 |

> **本质理解**：这四种边的区分只需要两个信息——`dfn[u]` 和 `dfn[v]` 的大小关系，以及 `v` 是否还在递归栈中。回边是强连通分量的"罪魁祸首"——只要存在回边，就一定形成环。横叉边在无向图 DFS 中不存在，这是无向图和有向图 DFS 最大的区别。

#### 2. Tarjan 算法——low 值就是"我能碰到的最早祖先"

**核心思想**：在 DFS 过程中维护两个值：
- `dfn[u]`：节点 u 被首次访问的时间戳（编号）
- `low[u]`：从 u 出发，经过若干条边（包括回边）能到达的仍在栈中的最小 dfn 值

当一个节点的 `low[u] == dfn[u]` 时，说明它和它栈上方的所有节点构成了一个**强连通分量**——因为谁也跳不出去。

```
// Tarjan 求 SCC 核心逻辑
void tarjan(int u) {
    dfn[u] = low[u] = ++timer;
    stack[++top] = u; in_stk[u] = 1;
    for each v in G[u]:
        if(!dfn[v]) {           // 树边：递归下去
            tarjan(v);
            low[u] = min(low[u], low[v]);  // 儿子能到的地方我也能到
        } else if(in_stk[v]) {  // 回边：直接更新
            low[u] = min(low[u], dfn[v]);
        }
    if(low[u] == dfn[u]) {      // 我是 SCC 的根
        ++scc_cnt;
        do { v = stack[top--]; in_stk[v] = 0; scc_id[v] = scc_cnt; }
        while(v != u);
    }
}
```

> **直觉**：`low[u]` 本质上是"以 u 为根的子树，通过若干条回边能'够到'的最远距离"。如果 low[u] < dfn[u]，说明能回到祖先——有环；如果 low[u] == dfn[u]，说明这个子树的"根"就是 u 自己——跑不出去的连通块就是 SCC。

> **low 值的精确定义与递推公式**。low[u] 的严格数学定义是：从 u 出发，**经过任意条树边（向下）后，再经过最多一条回边（向上）**，所能到达的仍在栈中的结点的最小 dfn 值。递推规则分三种情况：
> - **树边 (u→v, v 未访问)**：先递归 v，然后 `low[u] = min(low[u], low[v])`。因为从 u 出发，经过树边 (u,v)，再通过 v 子树中的回边能到达的祖先，u 也能到达。
> - **回边 (u→v, v 在栈中)**：`low[u] = min(low[u], dfn[v])`。注意这里用 `dfn[v]` 而非 `low[v]`——因为只能使用一条回边，不能"复用" v 子树中可能存在的第二条回边（否则会错误地把不属于同一 SCC 的结点归到一组）。
> - **横叉边 (v 已出栈)**：忽略。因为 v 已经归属某个 SCC，不能再通过它回到当前栈中。
...
- **割点**：删掉这个点后图不再连通。判定条件：
  - 根节点：至少有 2 个儿子（DFS 树中）
  - 非根节点：存在一个儿子 v 使得 `low[v] >= dfn[u]`（v 无法绕过 u 到达 u 的祖先）
- **桥（割边）**：删掉这条边后图不再连通。判定：`low[v] > dfn[u]`（v 无法绕过这条边到达 u）

> **关键区别**：割点用的是 `>=`，桥用的是 `>`——因为割点允许 v 通过另一条路径绕回到 u 本身，而桥要求 v 完全离开 u 的子树后也碰不到 u。

> **为什么根节点和非根节点的割点判定不同？** 对于非根节点 u，如果存在子节点 v 使得 `low[v] >= dfn[u]`，说明 v 及其后代无法绕过 u 回到 u 的祖先——一旦删除 u，v 的子树就与原图的其余部分断开，所以 u 是割点。但对于 DFS 树的**根节点 root**，它没有祖先可回，`low[root]` 始终等于 `dfn[root]`，所以 `low[v] >= dfn[root]` 对任何子节点都成立（只要 root 有子节点）。因此根的判定不能用 low 条件，而要改为：**根节点在 DFS 树中至少有两个子节点（child ≥ 2）才构成割点**。这背后的物理含义是：如果根只有一个子节点，删除根后剩下的部分仍然通过子节点的子树连通；如果有两个以上子节点，根的删除会使不同子树彼此分离。

#### 4. 2-SAT——蕴涵图上的 SCC 判定

2-SAT 问题：给定 (A ∨ B) 形式的一堆逻辑约束，每个变量只有真/假两种取值。建图规则：`(A ∨ B)` 等价于 `(¬A → B) ∧ (¬B → A)`，即"不选 A 就必须选 B，不选 B 就必须选 A"。这就是**蕴涵图**。

```
// 2-SAT 建图
// 变量 i：结点 2*i（真）、2*i+1（假）
// 子句 (A ∨ B) → 添加两条边：¬A→B, ¬B→A
```

判定方法：对蕴涵图跑 Tarjan 求 SCC。如果某个变量的真结点和假结点在同一个 SCC 中（`x` 和 `¬x` 互相可达），则无解；否则有解。赋值时，对每个变量选择拓扑序靠后的那个结点所代表的取值。

> **直觉**：`¬A → B` 的意思是"如果你不选 A，就必须选 B"。把所有这些"必须"连成图，SCC 中的结点是同生共死的——要么全选要么全不选。如果 x 和 ¬x 同在一个 SCC，矛盾就产生了。

> **2-SAT 建图的标准套路**：每个布尔变量拆成两个点——`2*i` 表示 x_i = true，`2*i+1` 表示 x_i = false。常见的约束形式及其建边规则如下：

| 约束 | 等价蕴含 | 添加的边 |
|------|----------|---------|
| x_i = true | — | `¬x_i → x_i`（2i+1 → 2i）|
| x_i = false | — | `x_i → ¬x_i`（2i → 2i+1）|
| x_i ∨ x_j | `¬x_i → x_j ∧ ¬x_j → x_i` | (2i+1→2j), (2j+1→2i) |
| ¬x_i ∨ ¬x_j | `x_i → ¬x_j ∧ x_j → ¬x_i` | (2i→2j+1), (2j→2i+1) |
| x_i → x_j | `x_i → x_j` | (2i→2j) |
| x_i ≠ x_j | `(x_i∨x_j) ∧ (¬x_i∨¬x_j)` | 前两条约束的组合 |

> 核心思路是：**蕴含关系 = 有向边**。所有约束都要先转化为"如果...则..."的蕴含形式，再用有向边编码。建图完成后，跑 Tarjan 求 SCC，若 x_i 和 ¬x_i 在同一 SCC 则无解；否则按拓扑序靠后的 SCC 赋值为真（即选 dfn 更大的那个值的含义）。
...
整条脉络的核心是 **dfn + low 这对时间戳**。理解了 low 值的物理含义，SCC、割点、桥的判定逻辑就是自然而然的事情了。

> **跨章关联**：DFS树的概念在全书多处扮演关键角色——**3.3-3.5节**中的AC自动机fail树本质就是一棵DFS树，fail指针连接方式与low值的"回溯"思想同源；**3.7节**中LCA的倍增算法依赖于DFS序（dfn时间戳），而树的双连通分量判定直接应用了本节割点/桥的low值判定逻辑；**1.4节**DP解决匹配问题时遇到的转移图和本节2-SAT的蕴含图都是"有向图的SCC缩点"思想的应用。

### 快速上手模板

```cpp
// 【Tarjan 求 SCC】有向图强连通分量 + 缩点
...
// 【2-SAT 判定】
// 变量 i: 2*i=true, 2*i+1=false
// 子句 (a ∨ b): add(¬a→b), add(¬b→a)
bool solve_2sat() {
    for (int i = 0; i < 2*n; i++)
        if (!dfn[i]) tarjan(i);
    for (int i = 0; i < n; i++)
