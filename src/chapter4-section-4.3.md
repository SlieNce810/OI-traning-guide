...
> **学习目标**：掌握四个经典计算几何算法框架，用排序和分治策略将朴素 O(N²) 优化到 O(N log N)

## 理论基础

### 为什么需要学这个？

给你 10000 个点，让你找"把所有点包住的凸多边形"。如果暴力枚举所有点对，检查每个点是否在某一侧——O(N³)，稳稳超时。如果再问"哪两个点距离最远"——O(N²) 都扛不住。计算几何的算法竞赛题，输入规模通常在 10⁴ 到 10⁵ 之间，**不掌握高效算法框架，连门槛都进不了**。

好消息是：二维几何的问题虽然看起来千奇百怪，但破解它们的核心技巧只有几个——排序预处理消除乱序，分治思想分解问题规模，以及利用几何特有的"对踵"性质将双层循环压成单层。学完这一节，你会掌握四种可以像模板一样直接套用的算法策略，让笨重的暴力搜索瘦身为优雅的对数级方案。

### 核心概念

**1. 凸包 — Andrew 算法（上下凸壳分别构造）**

朴素思路：检查每个点是否在任意三点构成的三角形内部 → O(N³)。Andrew 优化：先按 x 坐标排序，然后从左到右扫描构造下凸壳，从右到左扫描构造上凸壳。维护一个栈，每次加入新点时，检查栈顶两点与新点形成的转向：**右转（Cross ≤ 0）就弹栈**。
...
关键理解：**排序消除了随机性**，让凸包的构造变成一趟扫描。时间复杂度 O(N log N)（排序主导）。

**为何上下凸壳恰好围成完整凸包？** 按 x 坐标排序后，所有点从左到右排列。下凸壳从左端点扫描到右端点，栈中始终维护"当前已处理点集的下凸壳"——用 `Cross ≤ 0` 弹栈保证了每一步的栈顶都是凸的（没有凹陷）。当扫描到达最右点时，下凸壳包含了从左到右的"下边缘"。同理，上凸壳从最右点往回扫描到最左点，收集的是"上边缘"。由于最左点和最右点必定在凸包上，两次扫描在这两点处自然"对接"——下凸壳以最右点结束，上凸壳从最右点开始，最终形成首尾相连的闭合凸多边形。去重（pop掉重复的最左/最右端点之一）后，上下凸壳拼在一起就是完整凸包。直观理解：用一根绳子从左到右"兜住"所有点的下沿，再从右到左"兜住"上沿，绳子收紧后的形状就是凸包。

**2. 半平面交 — 排序 + 双端队列**

n 条直线的"公共区域"怎么求？朴素取两两交点再接上 → O(N²)。优化：按极角排序，用双端队列维护当前有效的半平面。每次加入新的半平面时，从队首和队尾弹出不再有效的旧半平面。

```cpp
sort(L.begin(), L.end());        // 按极角排序
q[first=last=0] = L[0];          // 双端队列初始化
for (int i = 1; i < n; i++) {
    while (first < last && !OnLeft(L[i], p[last-1])) last--;   // 从队尾弹出
    while (first < last && !OnLeft(L[i], p[first])) first++;   // 从队首弹出
    q[++last] = L[i];
    // 平行且同向的边，保留更内侧的那条
    if (fabs(Cross(q[last].v, q[last-1].v)) < eps) { last--; ... }
    if (first < last) p[last-1] = GetLineIntersection(q[last-1], q[last]);
}
```

双端队列的妙处：半平面交是一个凸多边形，加入新半平面可能在两端切掉顶点，所以队首队尾都需要检查。排序 O(N log N)，去冗余 O(N)。

**半平面交算法的正确性证明框架**：极角排序保证半平面按方向单调递增排列（从某个起始角绕一圈回到原点）。算法的核心不变式为：**在处理的任意时刻，双端队列 q[first..last] 中存放的半平面序列的交集，恰好等于当前已处理的所有半平面的交集**。证明要点：① 初始化时只有第一个半平面，交即为自身，不变式成立；② 加入新半平面 L[i] 时，从队尾和队首弹出点（用 `OnLeft` 检查）——被弹出的点必定落在新半平面的外侧（或线上），因此不再属于交集；③ 对于平行且同向的半平面，保留更内侧（更严格）的那条，因为外侧的那条对交集无贡献。循环结束时，队列中剩余半平面的交集就是最终答案。该算法本质上是"凸多边形切割"的扫描线版本——每加入一个半平面相当于用一条直线切割当前凸多边形，极角排序保证了切割线以旋转顺序出现，双端队列则高效维护了切割线的有效范围。

**3. 旋转卡壳 — 对踵点的 O(N) 性质**

给定凸包，求最远点对。"对踵点对"指被两条平行线夹住的凸包顶点对。旋转卡壳的关键性质：当平行线旋转一圈回到原点时，**每条边只需要检查常数个对踵点**——因为对踵点是单调移动的。

```cpp
// 对凸包的每条边 p[u]-p[u+1]，维持最远对踵点 p[v]
for (int u = 0, v = 1; u < n; u++) {
    while (Cross(p[u+1]-p[u], p[v+1]-p[v]) > 0)  // 面积还在增长
        v = (v + 1) % n;                           // 移动对踵点
    ans = max(ans, Dist2(p[u], p[v]));             // 记录距离
}
```

判断条件：`Cross(p[u+1]-p[u], p[v+1]-p[v]) ≤ 0` 意味着以当前边为底时，p[v] 到 p[u+1] 的距离不再增加，此时 p[v] 就是对踵点。总复杂度 O(N)。

**旋转卡壳单调性证明**：为什么对踵指针 v 只需单方向移动而不需要回溯？关键在于夹角的变化是单调的。设当前卡壳边为 p[u]p[u+1]，已找到对踵点 p[v]。当卡壳边前进到下一条边 p[u+1]p[u+2] 时，新边的方向相对于旧边逆时针旋转了一个正角度（凸包边的转向角）。此时原先的对踵方向也需要同步逆时针旋转才能维持"平行支撑线"状态——而凸包顶点是逆时针排列的，所以对踵点 v 只会单调地沿着凸包向前移动（即 `v = (v+1) % n`）。用一个三角形面积的比较来检测：`Cross(p[u+1]-p[u], p[v+1]-p[v])` 衡量的是"从当前对踵方向再往前走一步所转过的夹角"，当这个值 ≤ 0 时说明 v 已经越过最优位置（面积开始下降），此时 p[v] 就是当前边的对踵点。由于凸包是逆时针的，v 每次只需检查常数个顶点（通常 1～2 个），总计 O(N)。这一性质来源于"支撑线的夹角单调变化"这一几何事实。

**4. 最近点对 — 分治 + 跨线候选常数绑定**

按 x 坐标排序后，递归求解左右半区的最小距离 d，然后只需检查距中垂线距离不超过 d 的带状区域。**关键性质**：在宽度为 2d 的带内，对于任何一点，最多只需检查与它 y 坐标差在 d 以内的点——而这些点最多只有 5 或 6 个。这保证了合并步骤仍是 O(N log N) 而非 O(N²)。

### 知识脉络

```
排序预处理（消除乱序，所有算法的基础）
    ├── Andrew 扫描（凸包，O(N log N)）
    │       └── 旋转卡壳（凸包直径/最远点对，O(N)）
    │               └── 凸包 + 旋转卡壳 = 点集直径问题
    ├── 极角排序 + 双端队列（半平面交，O(N log N)）
    │       └── 二分 + 半平面交（最大内切圆半径判定）
    │               └── 半平面交判定可行域非空
    └── x 坐标分治（最近点对，O(N log N)）
            └── 跨线候选常数绑定（合并仅 O(N)）
```

这四种算法策略可以组合使用：凸包是旋转卡壳的前提，半平面交可以配合二分解决判定问题，分治思路则启发我们遇到"N 很大"时优先考虑能否拆分子问题。

**本书跨章节连接**：半平面交的判定性思维与第 4.2 节（圆的判别式）和第 1.2 节（二分答案）形成了"二分+判定"的黄金模板——用二分确定目标范围的上下界，用判定函数（半平面交非空/判别式符号/哈希相等）检验中点是否可行。这在"离海最远的点"（4.3 节例题10）和"最大内切圆"问题中得到了淋漓尽致的展示。凸包作为二维几何万能的预处理步骤，也是旋转卡壳、最远点对、最小矩形覆盖等高级算法的基础数据结构。最近点对的分治思想与第 3.9 节的 CDQ 分治结构一致——都是"切分→递归→跨区合并"。kd-Tree（3.10 节）与凸包在最远点对问题上分别提供了"动态近似"和"静态精确"两种不同的解决方案。

### 快速上手模板

```cpp
// Andrew 凸包 — 最通用的模板，直接传引用复用
...
这个凸包模板可以被旋转卡壳、最小矩形覆盖、最远点对等大量问题直接复用。注意两个 `<=` 改 `<` 可以控制凸包边上是否保留共线点。
