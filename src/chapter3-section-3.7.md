...
> **学习目标**：掌握树上的三大经典策略——倍增法（空间换时间）、树链剖分（把树上路径映射为线段树区间）、点分治（用重心分解实现 O(N log N) 的全树路径统计）——并理解它们各自的复杂度来源。

## 理论基础

### 为什么需要学这个？

树的题目有个共同的"气质"：问题描述很直观（"求两点间距离""统计路径对数"），但朴素做法一碰到大数据就歇菜。你会慢慢形成一种直觉：**树上的很多问题，之所以难，是因为路径不连续**。一维数组上的区间操作，你可以直接用线段树搞定；但树上路径是弯弯绕绕的，你没法直接拿一维数据结构往上套。这一节的三板斧，就是为了解决"如何把树上问题展平/切开/分解"这个核心矛盾：树链剖分把树"切"成 O(log N) 条连续的 DFS 区间，点分治把树"拆"成 O(log N) 层规模递减的子问题，倍增法则直接让你在"高度维"上用二进制加速跳跃。

### 核心概念

#### 1. 倍增 LCA：二进制 + 祖先快速跳转

**一句话定义**：`up[u][i]` 表示从节点 u 向上跳 2^i 步到达的祖先节点。任意跳跃距离 d 可以通过 d 的二进制拆解为至多 log N 步。

**本质理解**：为什么"二进制拆分"这么优雅？因为任何自然数 d 都可以唯一地表示为若干个互不重叠的 2 的幂次之和（这恰恰是二进制表示的本质）。所以从 u 向上跳 d 步 = 依次跳 `2^k1, 2^k2, ...` 步。倍增预处理就是提前把你可能跳的所有"幂次步"都算好。查询 LCA 时先把深的节点提升到同一高度（二进制拆 diff），再一起向上试探跳跃（从大到小试，不重合就跳）。

**与朴素对比**：朴素逐级上跳 O(N)；倍增 O(log N)。

#### 2. 树链剖分：为什么能切成 O(log N) 段？

**一句话定义**：把树的边分为重边（连向最大子树）和轻边。每条重链上节点 DFS 序连续，任意路径被切成不超过 O(log N) 条重链线段。

**证明思路**：每次走过一条轻边，子树大小至少减半（因为轻儿子的子树 ≤ 父节点子树的一半，否则它就是重儿子）。所以根到任意叶子的轻边数 ≤ log N。一条路径由两条"自上而下"的路径拼接而成，每段的分段数和轻边数同级，所以总段数 ≤ 2 log N。

**本质理解**：树链剖分做的就是把"弯弯绕绕的树上路径"翻译成"若干段连续的 DFS 序区间"，每段区间都可以丢给线段树处理。这是编译原理中"树展平为线性序列"思想的直接应用。

#### 3. 点分治：为什么重心的递归深度是 O(log N)？

**一句话定义**：重心是删除后使最大连通块最小的节点。每次选重心做根，统计经过重心的路径，然后递归进入各子树。

**本质理解**：重心的定义意味着——删除重心后，最大的连通块大小 ≤ N/2。因此每层递归，问题规模至少减半，递归深度 ≤ log N。这是分治策略在树上的完美体现：在一维数组上分治简单（直接对半分），在树上分治靠的就是"找重心"这一个操作。

**证明**：若重心 c 的存在一个子树的 size > N/2，则把 c 移到该子树中可以将最大连通块缩小，与 c 是重心的定义矛盾。

#### 4. 树链剖分的重链性质严格证明

**定理**：在轻重链剖分中，任意节点到根的路径上，经过的轻边数量不超过 log₂ N，因此路径被切成的重链段数不超过 O(log N)。

**证明**：设节点 u 有一条轻边连向父节点 fa(u)，即 u 不是 fa(u) 的重儿子。由重儿子的定义，重儿子是 fa(u) 的子树中 size 最大的那个子节点。因为 u 不是重儿子，所以存在 fa(u) 的另一个子节点 v（即重儿子），满足 size[v] ≥ size[u]。因此 fa(u) 的子树大小 size[fa(u)] ≥ size[u] + size[v] + 1 ≥ 2 × size[u] + 1 > 2 × size[u]。也就是说，**每经过一条轻边向上走，子树大小至少翻倍**。因为整棵树大小最多为 N，一条从叶到根的路径上至多有 log₂ N 条轻边。路径上的各段重链由轻边分隔，因此重链的段数 = 轻边数 + 1 → 也是 O(log N)。这就是为什么树链剖分中路径查询的线段树访问次数为 O(log² N)（每条重链一次线段树查询 O(log N)，共 O(log N) 条重链）或在优化实现下可以达到 O(log N)。

#### 5. 点分治的重心性质严格证明

**定理**：一棵树必然存在一个节点（重心），删除它后形成的所有连通块的大小都不超过原树大小的一半（N/2）。

**证明**：任选一个节点作为起点，沿子树大小最大的方向移动。设当前节点为 u，若 u 的某棵子树 v 满足 size[v] > N/2，则将"当前点"从 u 移动到 v。移动后，新节点 v 的"上方连通块"大小 = N - size[v] < N - N/2 = N/2（因为 size[v] > N/2），而 v 原来的子树中最大的那个（v 的重儿子）大小 ≤ size[v] - 1（因为子树不包括 v 自身，而 v 原先的子树大小可能接近 size[v]，但经过移动后，v 的新"最大连通块"是 max(N - size[v], max_child_size(v))。由于 N - size[v] < N/2，且 max_child_size(v) ≤ size[v]/2（否则 v 的重儿子会更大，我们会继续移动），所以 v 的所有连通块都 ≤ N/2。由于树的节点数有限，这个移动过程必定终止于一个重心。**推论**：点分治每层递归将问题规模至少减半，因此递归深度 ≤ log₂ N，总复杂度 O(N log N)（当每层处理 O(N) 时）。
...
树路径问题
    │
    ├──→ 两点间距离/祖先关系 ──→ 倍增LCA (O(log N)查询)
    │
    ├──→ 路径修改/查询(在线) ──→ 树链剖分
    │      把路径切成O(log N)段 ──→ 线段树维护
    │
    └──→ 全树路径统计(离线) ──→ 点分治
           每层选重心(O(log N)层) ──→ 各种组合统计技巧
```

**本书跨章节连接**：树是图论（第 5.1-5.4 节）的基础——有根树的 DFS 序、路径问题、连通性判断都贯穿了图论部分。**倍增法**在树的 LCA 问题中首次完整展示（3.7 节），而后反复出现：第 3.5 节（SAM）用倍增定位子串对应节点、第 3.11 节（可持久化）用树上倍增定位历史版本的祖先。**树链剖分**将树映射为线段树区间，直接连接了第 3.2 节（线段树）的设计思想。**点分治**的"选重心—分治—合并"范式与 CDQ 分治（3.9 节）"时间中点—左贡献右—递归"的结构高度对称——前者在空间（树结构）上分治，后者在时间维度上分治。

### 快速上手模板

```cpp
// 倍增LCA模板
int up[N][LOG], dep[N], tin[N], tout[N], timer;
void dfs(int u, int fa) {
    tin[u] = ++timer, up[u][0] = fa, dep[u] = dep[fa] + 1;
...
// 树链剖分模板
int sz[N], hson[N], top[N], dfn[N]; // hson: 重儿子, top: 重链头
void dfs1(int u, int fa) { /* 计算 sz 和 hson */ }
void dfs2(int u, int tp) {
    dfn[u] = ++timer, top[u] = tp;
    if (hson[u]) dfs2(hson[u], tp);  // 先走重儿子: 保证同链dfn连续
    for (int v : G[u]) if (v != fa && v != hson[u]) dfs2(v, v);
}
// 路径查询
while (top[u] != top[v]) {
