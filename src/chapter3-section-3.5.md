...
> **学习目标**：从 endpos 等价类这一核心直觉出发，理解后缀自动机（SAM）的结构本质——它到底是什么、为什么节点数是 O(N) 的、以及后缀链接树如何成为一切 SAM 高级应用的底座。

## 理论基础

### 为什么需要学这个？

后缀自动机可能是字符串算法中最让人"望而生畏"的东西之一。代码不长（三四十行），但每个变量都有看似玄学的含义，网上的教程大多直接丢代码然后说"这就是模板，背下来就行"。问题是，不理解为啥 `len[p]+1 == len[q]` 就不用克隆节点，不明白后缀链接到底构成了什么结构，你就永远调不好 SAM。这一节，我们把 SAM 拆开揉碎了讲——先从 **endpos 等价类** 这个最原始的概念入手，帮你建立直觉，然后顺藤摸瓜推导出后缀链接树，最后用一个简明的思路证明"节点数为什么不超过 2N"。

### 核心概念

#### 1. endpos 等价类：一样的"出现位置集合"形成同一个状态

**一句话定义**：`endpos(s)` 是子串 s 在 S 中所有**结束位置**的集合。如果两个子串的 endpos 集合完全相同，它们就属于 SAM 中的同一个节点。

**最小示例**：`S = "ababa"`。子串 `"ba"` 出现在位置 [3, 4]，结束位置集合 {3, 5}（1-based）。子串 `"aba"` 也出现在位置 [1, 3]，结束位置 {3, 5}。所以 `"ba"` 和 `"aba"` 有相同的 endpos 集合，它们在 SAM 中对应同一个状态。

**本质理解**：endpos 决定了"在这个位置出现之后，下一个字符可以是什么"。如果两个子串有相同的 endpos 集合，那它们后面跟的字符集合也相同——换句话说，从自动机匹配的角度看，它们是等价的，到达的是同一个状态。这是 SAM 状态**最少的**必要条件——每个 endpos 等价类就是自动机的一个状态。

#### 2. 后缀链接树（parent tree）

**一句话定义**：`link[u]` 指向 endpos 集合**真包含** u 的 endpos 集合、且长度最长的那个状态。所有 link 边构成一棵以初始状态为根的树。

**本质理解**：后缀链接的本质是"去掉开头的字符"。如果你从状态 u 沿 link 往上走，你得到的所有子串**恰好是那些出现在所有 u 出现过的位置上的、更短的后缀**。后缀链接树的父节点 = 更短的"超集"。另一层理解：后缀链接树是**将原串的所有后缀按前缀关系组织起来的树**，这就是为什么很多 SAM 问题要在这棵树上做 DP——你在树上的一个节点，endpos 集合的大小恰好等于它子树中所有前缀终点的个数。

#### 3. 节点数不超过 2N

**一句话证明思路**：SAM 只有两种节点——每次插入字符新建的节点（cur）和分裂节点（clone）。cur 类节点有 N 个。关键在 clone 节点：每次分裂产生一个新 clone，但分裂的同时 `link[cur]` 从指向某个旧节点改为指向 clone——这意味着 clone 不会新建 link 边，它只是"劫持"了已有的 link 关系。总节点数 ≤ 2N-1，总边数 ≤ 3N-4。

#### 4. endpos 等价类图示：以 S="ababa" 为例

**完整分类**：以 S = "ababa"（位置 1-based）为例，所有子串及其 endpos 集合如下：

位置 1 结尾的子串：{"a"} → endpos={1}
位置 2 结尾的子串：{"b", "ab"} → 两者 endpos={2}（注意 "ab" 也在位置 2 结束）
位置 3 结尾的子串：{"a", "ba", "aba"} → endpos={3}
位置 4 结尾的子串：{"b", "ab", "bab", "abab"} → 注意 "b" 也在位置 2 结束，所以它的 endpos 实为 {2,4}；同理 "ab" 的 endpos={2,4}
位置 5 结尾的子串：{"a", "ba", "aba", "baba", "ababa"} → "a" 出现在 {1,3,5}；"ba" 出现在 {3,5}；"aba" 出现在 {3,5}

**等价类合并结果**：最终 endpos 相同的子串归为一个 SAM 状态：
- endpos={1,3,5} 对应子串 {"a"} → 状态 A
- endpos={2,4} 对应子串 {"b", "ab"} → 状态 B（注意 "b" 和 "ab" 的 endpos 相同）
- endpos={3,5} 对应子串 {"ba", "aba"} → 状态 C
- endpos={5} 对应子串 {"baba", "ababa"} → 状态 D

**图示规律**：从叶到根，endpos 集合越来越大（子串越来越短），这正是后缀链接树的结构——从特定状态沿 link 上行等同于向 endpos 集合中添加更多的出现位置。

#### 5. SAM 状态分裂的直观解释

**什么时候需要分裂？** 插入字符 c 时，从 last 沿 link 链向上走，给所有没有 c 转移的节点添加转移到 cur。如果遇到某个节点 p 已有 c 转移指向 q，此时需要判断：(1) 若 len[p]+1 == len[q]，说明 q 恰好比 p 多一个字符 c，q 自然满足所有条件，无需分裂；(2) 若 len[p]+1 < len[q]（即 len[q] > len[p]+1），说明 q 表示的等价类中包含了两个不同长度的子串——较短的那些在 p 加字符 c 后产生，较长的那些不是。

**分裂操作的含义**：创建 clone 节点 nq，copy q 的所有转移，len[nq]=len[p]+1（nq 表示"p 加字符 c 后恰好产生的那些子串"的等价类）。然后 link[q]=nq（q 变为 nq 的后缀），link[cur]=nq（新节点 cur 的后缀也是 nq）。最后沿 link 链向上，把所有原本指向 q 的 c 转移改为指向 nq。**分裂的本质是将一个 endpos 等价类拆成两个**：nq 继承了原 q 的较短部分（endpos 集合更大），而 q 保留了较长部分（endpos 集合是 nq 的子集）。这就是为什么节点数最多 2N——每次分裂产生一个新节点，但最多分裂 N-1 次。
...
S的子串分类
    │
    ▼
endpos等价类 ──→ SAM节点(状态) ──→ 转移边(DAG) ──→ 本质不同子串数 = Σ(len[u]-len[link[u]])
    │
    ▼
后缀链接树(parent tree)
    │
    ├──→ endpos合并(线段树合并) ──→ 第k次出现
    ├──→ 树DP(计数) ──→ 出现次数统计
    └──→ 最长公共子串匹配
```

**本书跨章节连接**：SAM 的**后缀链接树（parent tree）**是一棵真正的树——第 3.7 节（树算法）中的树链剖分、线段树合并、倍增等技巧可直接应用于这棵树上。在第 5.2 节（图论）中，AC 自动机的 fail 指针构成的树与 SAM 的后缀链接拥有同构的结构——两者都是"失配/后缀"关系的组织。从 KMP 到 AC 自动机到 SAM，这条"自动机进化线"是字符串算法的核心脉络。
...
// SAM 核心（只展示关键逻辑）
struct SAM {
    int link[SZ], len[SZ], ch[SZ][SIGMA], last, sz;
    void init() { sz = 0, last = new_node(); }
    int new_node() { /* 初始化 ch, link=-1, len=0 */ return sz++; }

    void insert(int c) {
        int cur = new_node(), p = last;
        len[cur] = len[p] + 1;        // 新节点的 maxlen
        // 沿 link 链给没有 c 转移的节点添加转移
        while (p != -1 && !ch[p][c]) ch[p][c] = cur, p = link[p];
        if (p == -1) { link[cur] = 0; return; }       // 情况1: 全空
        int q = ch[p][c];
        if (len[p] + 1 == len[q]) { link[cur] = q; return; } // 情况2: 无需分裂
        // 情况3: 分裂 clone
        int nq = new_node();
