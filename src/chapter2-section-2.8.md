...
> **学习目标**：理解多项式从系数表示到点值表示的转换，掌握FFT加速卷积运算的核心原理与模板。

## 理论基础

### 为什么需要学这个？

给你两个长度10^5的数组，要求它们每一项两两乘积的和——朴素O(n^2)直接超时。但如果你听说过"卷积"，就会知道这是FFT的天生主场。FFT是算法竞赛中最"硬核"的数学工具之一，初次接触时复数运算、单位根、蝴蝶变换这些概念会让人望而却步。但请放心，本节的目标不是让你成为数值分析专家，而是帮你建立FFT的直观理解并熟练掌握模板——知道什么时候该用它、怎么用预写的代码解决问题。你只需要理解"系数 → 点值 → 相乘 → 系数"这个基本流程就够了。

### 核心概念

**多项式表示：系数 vs 点值**：一个n次多项式有两种等价的表示方式。系数表示：A(x) = a_0 + a_1·x + ... + a_{n-1}·x^{n-1}，用[a_0,...,a_{n-1}]描述。点值表示：取n个不同的x值，记录每个x对应的A(x)值，用[(x_0, y_0), ..., (x_{n-1}, y_{n-1})]描述。关键洞察：**系数表示下两个多项式相乘需要O(n^2)，但在点值表示下只需O(n)**——对应点值相乘即可（因为 (A·B)(x) = A(x)·B(x)）。FFT的本质就是快速地在系数和点值之间转换。

**单位根的性质**：FFT在复数域上工作，选取的"x值"是单位根 ω_n = e^{2πi/n}，它满足 ω_n^n = 1。单位根的核心性质：(1) 折半引理：ω_{2n}^{2k} = ω_n^k（偶数次方的单位根规模减半）；(2) 消去引理：ω_n^{k+n/2} = -ω_n^k（半圈恰好相反）。这两个性质让FFT可以用分治策略递归地将规模减半。

**复数根的几何直觉**：在复平面上，ω_n^k = e^{2πi·k/n} 对应的是**单位圆上等分点的坐标**。取n=8为例，ω_8^0=1 在正右方，ω_8^1在右上45°，ω_8^2= i在正上方，以此类推逆时针排列。这n个点构成一个正n边形。关键洞察：(1) 取n个点中偶数编号的点（ω_n^0, ω_n^2, ω_n^4, ...），恰好是n/2个单位根（即ω_{n/2}^0, ω_{n/2}^1, ...）——这就是"折半引理"的几何意义：偶数次方点把正n边形降级为正n/2边形。(2) ω_n^{k+n/2} 和 ω_n^k 关于原点对称（差半圈），所以恰好互为相反数——这解释了消去引理。FFT的分治就是利用这个几何事实：用n个点做插值的问题，可以递归为用n/2个点做插值的两个子问题（一个处理偶数次项，一个处理奇数项），然后在合并时利用相反数的关系做蝴蝶操作。复数域看似抽象，但单位根只是一个正多边形顶点集合而已。

**蝴蝶变换**：FFT的分治实现需要一个巧妙的"位逆序置换"（bit-reversal permutation）——把原序列按下标的二进制反转重新排列。为什么？因为分治过程中偶数项和奇数项被分开，经过log n层后，最终位置恰好是其原始下标二进制反转后的位置。蝴蝶变换是FFT实现中"先重排后合并"策略的关键一步。

**从DFT到FFT：分治如何把N²变成NlogN**：直接按定义计算离散傅里叶变换(DFT)，每个系数a_j需要与每个单位根 ω_n^{jk} 相乘再累加，n个系数产生O(n²)的计算量。FFT的核心创新在于利用单位根的性质**避免重复计算**。在多项式 A(x)=a_0+a_1x+a_2x²+...+a_{n-1}x^{n-1} 中，分离偶数项和奇数项：A(x) = (a_0+a_2x²+...+a_{n-2}x^{n-2}) + x·(a_1+a_3x²+...+a_{n-1}x^{n-2}) = A_even(x²) + x·A_odd(x²)。现在，A(x)在n个点(ω_n^0,...,ω_n^{n-1})处的值，可以复用A_even和A_odd在n/2个点(ω_{n/2}^0,...,ω_{n/2}^{n/2-1})处的值。因为x²的取值恰好从n个降为n/2个（折半引理）。这就是分治的基本递归：T(n) = 2T(n/2) + O(n)，解开得到T(n) = O(n log n)。每层递归合并时执行的"蝴蝶操作"——A[k]=A_even[k]+ω_n^k·A_odd[k], A[k+n/2]=A_even[k]-ω_n^k·A_odd[k]——本质上就是利用 ω_n^{k+n/2} = -ω_n^k 这个对称性，一次算两个结果。

**卷积的滑动窗口直觉**：离散卷积 c_k = Σ_{i+j=k} a_i·b_j 的实际含义可以用"滑动窗口"来理解。(1) 先将序列b翻转，使得 b'_i = b_{n-1-i}。(2) 然后将b'沿着a滑动：当b'的末尾对齐a的开头时，重叠区域的对应乘积和就是c_0；当b'右移1位，新的乘积和就是c_1；依此类推。(3) 遍历所有可能的对齐位置(k=0到2n-2)，就得到了完整的卷积序列。这种直觉解释了两类经典应用：大整数乘法中，每一位数字的"进位贡献"恰好对应滑动对齐时的乘积和；在模式匹配中，卷积结果c_k的"峰值"恰好对应模式串b在文本a中位置k处的最佳匹配——FFT正是用O(n log n)时间完成了这个n个滑动窗口的所有内积计算。

**卷积的定义与应用**：两个序列a和b的卷积 c_k = Σ a_i·b_{k-i}（离散卷积）。常见应用场景：(1) 大整数乘法——把一个数的各位作为多项式系数，乘积的各位就是卷积的结果；(2) 数组任意两项和的分布——统计所有A[i]+A[j]=v的个数，用FFT后查卷积结果；(3) 模式匹配——用FFT可以O(n log n)做带通配符的字符串匹配。

### 知识脉络

```
多项式系数表示  ──→  点值表示(FFT)  ──→  点值相乘  ──→  逆FFT还原
     │                    │
     └── 单位根  ←── 分治递归(折半引理)
                         │
                         └── 蝴蝶变换(位逆序)
...
FFT的核心流程只有三步：FFT(系数→点值)、逐点相乘、IFFT(点值→系数)。单位根的性质保障了分治的正确性，蝴蝶变换保障了O(n log n)的效率。对竞赛选手来说，更实际的是理解卷积的应用场景——当你遇到"两个序列两两组合后某个值出现的次数"这类问题时，要能反应出这是卷积。本节中FFT的分治递归结构（T(n)=2T(n/2)+O(n)）与第1.2节分治法中的Master定理完全一致，是分治思想的最高阶应用；**卷积计数**在第2.1节中作为原理被介绍，而FFT是其大规模计算的工程实现；**NTT（数论变换）**则直接依赖第2.3节中的原根和模运算知识。
...
void fft(vector<cd> &a, bool invert) {
    ...
    for (int len = 2; len <= n; len <<= 1) {
        ...
        cd wlen(cos(ang), sin(ang));  // 当前层的单位根
