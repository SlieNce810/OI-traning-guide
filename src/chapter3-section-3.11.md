...
> **学习目标**：掌握可持久化的核心思想——"路径复制"（每次修改只新建 O(log N) 个节点），并理解这一思想在主席树（区间第 K 小）、可持久化 Trie（最大异或）中日臻成熟的应用模式。

## 理论基础

### 为什么需要学这个？

你可能遇过这样的需求：一个数组，每次修改都生成一个新版本，但你想随时查询任意历史版本的状态。直接复制一遍数组？1000 个版本就是 1000 倍的内存开销——不行。这就是"持久化"要解决的问题：**让每次修改只产生 O(log N) 的新节点，其余结构全部共享**。这个思想一旦内化，你会发现它几乎可以嫁接在任何树形数据结构上——线段树、Trie、平衡树……全都支持"加一维版本号"。但关键是你要理解：到底是什么让 O(log N) 成为了可能？为什么是"路径复制"而不是"全量复制"？

### 核心概念

#### 1. "路径复制"：为什么每次只建 O(log N) 个节点？

**一句话定义**：每次插入/修改操作，只新建从根节点到目标叶子节点这条路径上的节点，路径之外的所有节点全部共享旧版本。

**本质理解**：考虑一棵线段树。如果你要修改叶子节点 x 的值，受影响的只有从根到 x 这条路径上的 O(log N) 个节点——其他节点（没有这条路径经过的节点）**根本感知不到变化**。因此你只需要"克隆"这条路径创建新节点，新节点的新子指针指向新路径上的子节点，而所有未被修改的子树指针全部指向旧版本的对应节点。这就是持久化 = **"写时复制"（Copy-on-Write）** 在线段树上的完美体现。

**空间代价**：N 次修改，每次 O(log N) 新节点，总空间 O(N log N)。对于 N=10^5，log N≈17，约 170 万节点——完全可控。

#### 2. 主席树：前缀版本相减 = 任意区间

**一句话定义**：VER[i] 是前 i 个元素的权值线段树。区间 [l, r] 的统计信息 = `VER[r] - VER[l-1]`（对应节点 count 相减）。

**本质理解**：这利用了权值线段树的加法可减性。两棵结构完全相同的权值线段树，如果把它们对应节点的 count 相减，得到的就是"只在区间 [l, r] 中出现的那些元素"构成的虚拟权值线段树。然后在这棵虚拟树上二分查找，就能找到第 K 小。整个过程 O(log N) ——不需要真正构建出这棵虚拟树，只需要同时导航两棵（或更多棵）树。

**最小示例**：序列 [3, 1, 4, 1]，查询区间 [2, 4] 的第 2 小。`VER[4]` 的 count 记录就是"全序列出现分布"，`VER[1]` 记录的是"仅第 1 个元素前的分布"。`VER[4].count - VER[1].count` = "区间 [2,4] 内各值的出现次数"。在这组 count 上二分：值域左半有 2 个元素 ≥ 2，答案在左半 → 继续，最终找到 1。

#### 3. 可持久化 Trie：按位贪心 + 版本差分

**一句话定义**：与主席树完全相同的模式——每个前缀构建一个 Trie（按二进制位从高到低插入），路径上的节点每次复制，`Trie(u) + Trie(v) - Trie(lca) - Trie(fa(lca))` 表示路径上的集合。

**本质理解**：Trie 的每个节点代表了二进制前缀的某种组合。`cnt` 记录该子树中有多少个值。当你需要"从路径集合中选一个值使 XOR 最大"时，可持久化 Trie 的四树差分让你知道"在目标集合中，某位能否选相反的"——只需要检查四棵 Trie 在相反位上的 cnt 之和是否 > 0。贪心决策：如果能走相反就反走（贡献 1<<bit），否则同走。

#### 4. 可持久化线段树的空间复杂度严格证明

**定理**：对一个大小为 N 的值域 [0, V) 建立可持久化权值线段树，依次插入 N 个元素，总节点数 = O(N log V)。每次插入操作恰好新建 O(log V) 个节点。

**证明**：线段树是一棵高度为 ceil(log₂ V) 的完全二叉树。插入一个值时，从根节点出发沿着唯一路径走到目标叶子节点，路径长度 = O(log V)。在可持久化版本中，路径上的每个节点都被"克隆"（新建一个节点），新节点的数据 = 旧节点数据 + 1（count+1），然后新节点的子指针中：沿路径方向的子指针指向新克隆的子节点，另一个子指针仍然指向旧版本的对应子树（共享）。**路径之外的节点完全不受影响**——它们既不需要修改 count（因为不在路径上），也不需要修改子指针（因为它们的子指针中不包含路径上的节点）。因此每次插入恰好新建 O(log V) 个节点。N 次插入的总节点数 = N × O(log V) + 初始空树的节点数。对于 V = 10^9（离散化后通常 V ≤ N），log₂ V ≈ 30；对于实际常见的 V ≤ 2×10^5，log₂ V ≤ 18。总节点数通常在 N × 18 到 N × 30 之间，完全可控。

**空间优化技巧**：使用数组模拟指针（而非 `new`），预分配 N × log V 大小的数组，避免动态分配的开销和内存碎片。

#### 5. 函数式编程与可持久化的关系

**核心联系**：可持久化数据结构的本质就是函数式编程中的"不可变数据结构"（persistent/immutable data structure）。在纯函数式语言（如 Haskell、OCaml）中，所有数据默认不可变——对一个数据结构的"修改"实际上创建了一个新的版本，旧版本保持不变。这正是可持久化线段树"每次修改只新建路径节点、其余共享"的思想来源。

**编程语言映射**：
- 函数式语言中的 List、Map、Set 默认就是可持久化的（如 Scala 的 immutable.Vector、Clojure 的 PersistentVector 内部使用类似 Trie 的结构实现 O(log₃₂ N) 的"修改"）。
- 在 C++ 等命令式语言中，我们需要手动实现 copy-on-write 来模拟这种效果——这正是主席树和可持久化 Treap 所做的。GCC 的 `__gnu_cxx::rope` 就是将函数式平衡树的思想封装成了 C++ 可直接使用的类型。
- **理解可持久化的另一个视角**：把它看成"数据结构版本的 git"。每次 commit（插入/修改）产生一个快照（版本根节点），不同快照共享未修改的部分（tree object 的 blob 共享），而 diff 只存储在改变的路径上（新创建的节点）。git 的对象模型和可持久化线段树的路径复制，本质上完全一致。
...
数据版本化需求
    │
    ▼
"路径复制"思想 (Copy-on-Write)
每次修改只新建 O(log N) 节点，其余共享
    │
    ├──→ 线段树 ──→ 主席树(区间第K小)
    │               VER[r] - VER[l-1] = 区间分布
    │
    ├──→ Trie ──→ 可持久化Trie(树上XOR最大)
    │             四树差分表示路径集合
    │
    └──→ Treap ──→ 可持久化Treap(文本编辑器)
                  copyOf + split/merge
```

**本书跨章节连接**：可持久化数据结构的"写时复制/路径复制"思想与第 1.4 节（DP）中的**状态保存**有共同之处——DP 中需要回退到历史状态时，我们可以保存关键时刻的状态以备恢复。可持久化将这种思路系统化为数据结构本身的能力——每个修改自动产生一个快照。主席树作为可持久化线段树，在"树上路径第K小"（3.7 节 + 3.11 节组合应用）中展示了数据结构跨领域联合的力量：倍增 LCA 定位路径端点，主席树用四树差分表示路径集合，二分查找确定第 K 小。可持久化 Trie 与第 5.2 节（图论）中的"Trie上的DFS"理念相通。
...
Node* insert(Node* old, int l, int r, int w) {
    if (l <= w && w < r) {
        if (l + 1 == r) return new Node{old->count + 1, Null, Null};
        int m = (l + r) / 2;
        return new Node{old->count + 1,
                        insert(old->left, l, m, w),
                        insert(old->right, m, r, w)};
    }
    return old;  // 不在路径上 → 直接返回旧节点（共享！）
}

int query(Node* a, Node* b, int l, int r, int k) {
    if (l + 1 == r) return l;
    int m = (l + r) / 2;
    int cnt = a->left->count - b->left->count;  // 版本差分
    if (cnt >= k) return query(a->left, b->left, l, m, k);
    return query(a->right, b->right, m, r, k - cnt);
}

// 可持久化Trie（XOR查询）
Node* insert(Node* old, int v, int dep) {
    Node* np = new Node(*old);  // 复制当前节点
    np->cnt++;
    if (dep >= 0) {
        bool c = v & (1 << dep);
        np->ch[c] = insert(old->ch[c], v, dep - 1);  // 新建路径
    }
    return np;
}
int query(Node* a, Node* b, int x, int dep = 15) {
    if (dep < 0) return 0;
    bool c = !(x & (1 << dep));
    if (a->ch[c]->cnt - b->ch[c]->cnt > 0)
        return (1 << dep) | query(a->ch[c], b->ch[c], x, dep - 1);
    return query(a->ch[!c], b->ch[!c], x, dep - 1);
}
```
