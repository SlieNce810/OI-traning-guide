...
> **学习目标**：掌握复杂几何问题的建模策略，学会用二分、扫描线组合几何工具，系统化地处理几何精度问题。

## 理论基础

### 为什么需要学这个？

很多ACM选手对几何有本能的恐惧——不是因为数学难，而是因为写完代码后调不出正确答案。明明算法思路是对的，但答案总差那么一点，玄学地改来改去。更深层的问题是：几何题和常规算法题完全不同，它需要你把"连续运动"翻译成"离散计算"。

本节的核心不是罗列向量运算公式（那些在基础章节已经会了），而是教你**几何问题的建模方法论**：

1. **连续化离散**：人和行李在凸多边形上持续运动，什么时候能相遇？答案是"二分时间t，每次判断行李在t时刻的位置，然后用最短路验证人能否在t时间内走到那个位置"。
2. **几何+二分**：几何判定比几何求解好写一万倍。把"求最早相遇时间"转化为"判断能否在t秒内相遇"，就是二分答案的本质。
3. **精度问题的系统化处理**：eps取多少、什么时候用dcmp、什么时候用long double、浮点数的比较用绝对值差还是相对差——这些不是玄学，是有一套工程化方案的。

学完这节，你就能用"二分答案+几何判定"的模式去解决一大类运动/碰撞问题，也能熟练处理多圆并集面积、山轮廓线、三维投影面积等经典几何题。

### 核心概念

**离散化连续运动**：当几何对象在连续移动时（如物体沿多边形边界运动），不要尝试求连续解析解。策略是：选取关键时间点（或二分枚举时间），在固定时刻把问题退化为"静态几何判定"。

```
朴素想法：对时间t列解析方程求解最早相遇 → 高维非线性，基本不可解
建模策略：二分时间t，在t时刻判断可达性 → 每次只需几何判定+最短路
```

*这本质上是用"判定"替代"求解"——判定通常只是几个叉积+射线法，而求解可能是不可积的微分方程。*

**精度控制的工程化方案**：
- `dcmp(x)` 用 `fabs(x) < eps ? 0 : (x < 0 ? -1 : 1)`，统一浮点比较
- eps 一般取 `1e-10`（double下），如果计算涉及开方或多层运算用 `1e-8`
- 两数相等用 `dcmp(a-b)==0`，绝对不用 `a==b`
- 线段相交用"规范相交"（叉积严格异号），排除端点重合
- 整数运算优先：能用 `long long` 叉积就不用 `double`

**扫描线+面积累积**：多个圆的并集面积、多个三角形的投影轮廓、天际线——这些问题共享同一个模式：在X方向离散化关键坐标（顶点+交点），相邻X之间通过扫描线确定"在此X条带内最高/最小的几何体"，用梯形/矩形近似累加面积。离散化越密越精确，用二分而非固定步长保证可控精度。

**旋转体/运动物体的离散化策略（时间切片法）**：当几何对象处于连续运动中（如旋转体、沿轨迹运动的物体），直接求解析解往往涉及不可积的超越方程。时间切片法提供了一种统一的离散化思路：将连续时间轴划分为有限个关键区间，在每个区间内运动轨迹近似为线性，从而将动态问题转化为逐段静态判定。具体步骤：①收集所有"运动方向或速度发生变化的时刻"作为关键时间点——对于旋转体，这些时刻对应"某条边扫过另一条边"的瞬时；对于沿多边形边界运动的物体，这些时刻对应"经过某个顶点"的瞬间。②在相邻关键时间点之间，所有几何对象的位置可以表示为时间的线性函数（或至少是单调函数），用二分法确定临界碰撞/相交时刻。③在每个关键区间内，用静态几何判定（如凸包交集、射线法）处理可达性和覆盖问题。这种策略在机器人路径规划（运动物体与障碍物的回避）、物理引擎中的碰撞检测（连续碰撞检测, CCD）以及LA2397收集行李问题中都有体现。关键在于选择合适的时间离散粒度——太粗会错过碰撞，太细则计算量爆炸，通常结合二分答案在判定精度和效率之间取平衡。

**扫描线在几何专题中的高级应用（面积交与周长并）**：扫描线不仅是"求n个矩形面积并"的标准工具，在更复杂的几何问题中还有以下应用模式：
- **矩形面积交**：将所有矩形的左右边作为事件排序，扫描每条垂直线段时维护当前被覆盖的Y区间。当且仅当某Y区间同时被k个矩形覆盖时，该区间对"至少被k个矩形覆盖的面积"有贡献。这需要在扫描线的每段Y区间上维护覆盖计数，且只有当计数 ≥ k 时才累加面积。数据结构上，使用线段树维护Y坐标离散化后的区间覆盖计数。
- **多边形周长并**：不仅累加面积，还需累加轮廓周长。在扫描线X方向推进时，维护当前Y方向被覆盖的总长度。当覆盖总长度从前一个X变化到当前X时，新增的垂直周长 = |变化量|。水平周长的处理则需要在每条事件的Y区间上，检查覆盖计数是否从0变为1（轮廓开始）或从1变为0（轮廓结束），在变化位置的Y坐标处累加水平线段长度。
- **多圆并集周长**：将每个圆的圆周分解为若干弧段（被其他圆遮挡的部分剔除），剩余弧段的端点即为"关键事件点"。在极角排序后逐段处理每段圆弧，累加弧长即可得总周长——本质上也是一种极角方向的"扫描线"。
...
几何建模策略
    → 连续运动离散化: 二分答案+静态几何判定
    → 几何+二分组合: 把"求解"转化为"判定"
    → 扫描线算法: X离散化 → Y事件排序 → 区间覆盖

精度处理方案
    → dcomp统一比较（eps=1e-10）
    → 整数叉积优先（long long）
    → 规范化相交判断（规范相交 vs 端点重合）
...
> **跨章关联**：本节几何专题的核心在于"建模策略"而非基础几何运算——**4.1-4.3节**覆盖了向量运算、位置判定、多边形的全部基础工具，本节在这些基础上构建了四个层次的进阶应用：①二分答案+几何判定（与5.3节最短路判定共享"将求解转化为判定"的思维模式）；②扫描线+面积累积（与6.2节分块的区间管理思想同源，都是以固定的离散化步长处理连续问题）；③齐次变换矩阵（与2.7节矩阵运算一脉相承）；④精度系统性管理（这是几何题区别于其他算法题的最独特挑战，4.1节基础的eps选取在本节得到工程化加强）。
...
// 二分答案+几何判定模板
bool check(double t) {
  Point target = compute_position(t);  // 计算t时刻目标位置
  构图：忽略被障碍物挡住的连线；
  Dijkstra最短路从起点到target；
  return shortest_dist <= max_speed * t;
}

double solve() {
  double L = 0, R = upper_bound;
  while (R - L > eps) {  // 或用秒数取整判断
    double M = (L + R) / 2;
...
核心要点：
1. 几何判定的正确性远比精度优化重要——先保证逻辑正确
2. `eps` 的选择取决于运算链长度和答案精度要求
3. 扫描线+离散化是最通用的几何求面积方法
4. 有整数坐标优先用整数运算避免精度陷阱
